// ============================================
// WSL 添加服务器视图模型 —— 与官方 packages/app/src/wsl/settings-model.ts 一一对应
// 全部为纯函数：输入 WslServersState，输出 UI 状态；i18n 以 key+params 表达，
// 由 React 渲染层翻译
// ============================================
...
export function isHiddenDistro(name: string) {
  return /^docker-desktop(?:-data)?$/i.test(name)
}

/** 判定是否为 WSL 服务器：后端注册 WSL 服务器时用确定性 id 约定 `wsl:<distro>`，
 *  前端（侧边栏徽标、设置页过滤等）据此识别，无需依赖 store 状态 */
export function isWslServerId(id: string): boolean {
...
export function addServerViewModel(input: {
  ...
  // 发行版列表正在后台刷新：runtime 已就绪但列表尚未到达时，
  // 用它区分「检测中」和「确实没有发行版」，避免冷启动时误报"尚未检测到"
  const distrosChecking = state?.job?.kind === 'distros'
...
// ============================================
// 自动探测计划（官方 autoProbePlan / addableProbePlan / addServerProbePlan）
// 弹窗的 effect 每次状态变化重算：该探什么、探什么 key，全部由纯函数决定
// ============================================
...
/** 失败门控（官方 createProbeFailureGate）：同 key 失败后不再自动重试，
 *  用户显式操作（checkAgain 等）时 reset 解除 */
